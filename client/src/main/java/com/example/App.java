...
public class App extends Application {
    ...
    private void handleCreateRoom() {
        ...
        dialog.showAndWait().ifPresent(result -> {
            if (result != null && result.name != null && !result.name.isEmpty()) {
                ...
                createPacket.setRoomName(result.name);
                createPacket.setMaxMembers(result.limit);
    ...
    private void showChat(Room room) {
        ...
        chatClient.sendPacket(new Packet(MessageType.CLIENT_READY));
    }
    ...
    private void sendFileChunks(File file) {
        try (InputStream inputStream = Files.newInputStream(file.toPath())) {
            ...
            while ((bytesRead = inputStream.read(buffer)) != -1) {
                ...
                if (!sendFileChunk(file, mimeType, fileId, chunkIndex, totalChunks, chunk)) {
                    Platform.runLater(() -> new Alert(
                            ...
                            "Koneksi ke server terputus saat mengirim file.").show());
                    return;
                }
                chunkIndex++;
            }
            ...
            Platform.runLater(() -> new Alert(
                    Alert.AlertType.ERROR,
                    "Gagal membaca file: " + e.getMessage()).show());
        }
    }

    private boolean sendFileChunk(
            File file,
            String mimeType,
            String fileId,
            int chunkIndex,
            ...
            switch (packet.getType()) {
                ...
                case ROOM_LIST:
                    if (lobbyView != null && packet.getMessage() != null) {
                        ...
                        for (String roomStr : packet.getMessage().split(",")) {
                            ...
                            if (parts.length >= 4) {
                                lobbyView.addRoom(new Room(
                                        parts[0],
                                        parts[1],
                                        parts[2],
                                        parseMaxMembers(parts[3])));
                ...
                case JOIN_SUCCESS:
                    showChat(new Room(
                            packet.getRoomId(),
                            packet.getRoomName(),
                            packet.getUsername() != null ? packet.getUsername() : currentUser,
                            packet.getMaxMembers()));
                    pendingRoom = null;
                    break;
                ...
                case ERROR:
                    new Alert(Alert.AlertType.WARNING, packet.getMessage()).show();
                    pendingRoom = null;
                    break;
    ...
    private int parseMaxMembers(String value) {
        try {
    ...
    private void handleMessageHistory(Packet packet) {
        if (chatView == null || packet.getMessage() == null) {
            return;
        }
    ...
    private static class IncomingFile {
        ...
        private IncomingFile(Packet p) {
            ...
            this.fileMimeType = p.getFileMimeType();
            this.fileSize = p.getFileSize();
            this.chunks = new byte[p.getTotalChunks()][];
        }

        private boolean addChunk(Packet p) {
            int chunkIndex = p.getChunkIndex();
            if (chunkIndex < 0 || chunkIndex >= chunks.length || p.getFileData() == null) {
                return false;
            }
